...
class DenonHeos extends events.EventEmitter {
	...
	send( command, qs, callback ) {
		...
		let sendQueueItem = {}
			sendQueueItem.command = `${command}?` + querystring.unescape(querystring.stringify( qs ));
	...
	*/
	...
	systemSignIn( username, password, callback ) {
	...
	*/
	...
	playerPlayPreset( playerId, presetId, callback ) {
		return this.send(`player/play_preset`, {
			pid: playerId,
			preset: presetId
		}, callback);
	}
	
	playerGetPlayMode( playerId, callback ) {  	
    return this.send(`player/get_play_mode`, {
      pid: playerId,
    }, callback);
	}
	
	playerSetPlayMode( playerId, { shuffle, repeat }, callback ) {  	
