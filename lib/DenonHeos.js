...
const PORT = 1255;
const TELLNET_PORT = 23;
const DELIMITER = '\r\n';
const TELLNET_DELIMITER = '\r';
...
class DenonHeos extends EventEmitter {
  ...
  */
  ...
  /*
   * Tellnet for AVR controls
   */

  async connectTellnet() {
    return new Promise((resolve, reject) => {
      this._socketTellnet = new Socket();

      this._socketTellnet
        .setTimeout(3000)
        .once('connect', () => {
          resolve('Tellnet connected');
        })
        .on('timeout', () => {
          this.debug('Tellnet socket onTimeout');
          this._socketTellnet.end();

          reject( new Error('Tellnet socket timeout') );
        })
        .on('error', err => {
          this.debug('Tellnet socket Error', err);
          reject(err);
        })
        .on('end', () => {
          this.debug('Tellnet socket onEnd');
        })
        .on('close', () => {
          this.debug('Tellnet socket onClose');

          this._socketTellnet.destroy();
          this._socketTellnet = null;

          reject(new Error('Tellnet closed'));
        })
        .on('data', ( data ) => {
          this._socketTellnet += data;
          this._parseTellnetData();
        })
        .connect( TELLNET_PORT , this.address );
    });
  }

  _parseTellnetData() {
    const tellnetBufferArr = this._tellnetBuffer.split( TELLNET_DELIMITER );
    if( tellnetBufferArr.length > 1 ) {
      const tellnetBufferItem = tellnetBufferArr.shift();
      this._tellnetBuffer = tellnetBufferArr.join( TELLNET_DELIMITER );

      if( tellnetBufferItem === 'PWSTANDBY' ) this.emit( 'avr_state' , false);
      if( tellnetBufferItem === 'PWON' ) this.emit( 'avr_state' , true);
    }
  }

  _writeTellnet( command ) {
    return new Promise((resolve, reject) => {
      if( this._socketTellnet ) {
        this._socketTellnet.once('data', result => {
          if( result ) {
            resolve(result.toString().replace(TELLNET_DELIMITER, ''));
          } else {
            reject(new Error('No result'));
          }
        })
        this._socketTellnet.once('error', err => {
          reject(err);
        })
        setTimeout(() => {
          resolve('OK')
        }, 200);
        this._socketTellnet.write(command + TELLNET_DELIMITER);
      } else {
  ...
  async getAvrState() {
    const state = await this._writeTellnet('PW?');
    return state.includes('PWON');
  }

  async setAvrState( state ) {
    if( state === true ) {
      const setState = await this._writeTellnet('PWON');
      if( setState ) {
      ...
      return await this._writeTellnet('PWSTANDBY');
    }
  }

  async getAvrInput() {
    return await this._writeTellnet('SI?');
  }

  async setAvrInput( input ) {
    return await this._writeTellnet(input);
  }
}
